...
### 2. 🎙️ 顺风耳 (Deep Speech Transcription)
*   **实时语音转录**：集成 **Sherpa-onnx** 深度学习模型，提供**毫秒级**的本地/云端语音转文字。
*   **全场景监听**：支持**系统内录音**（听网课/会议）和**麦克风输入**（听面试官提问）。
*   **智能分析**：听到关键信息直接生成摘要或答案，无需手动打字。

### 3. 👻 幽灵模式 (Stealth & Camouflage)
...
| 场景 | Q-Solver 能做什么？ |
| :--- | :--- |
| **💻 远程面试** | 实时转录面试官的问题，根据简历和题库实时提示答案，助你从容应对。 |
...
为了启用语音转录功能，您需要手动下载离线模型文件。**请务必仔细阅读以下兼容性说明，否则程序将无法启动。**

1.  **访问模型仓库**：
    前往 [Sherpa-onnx ASR Models Releases](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models)。

2.  **下载模型 (推荐)**：
    请找到并下载 **zipformer** 系列模型
    * `sherpa-onnx-streaming-zipformer-zh-xlarge-int8-2025-06-30.tar.bz2`
    

3.  **配置与重命名 (关键步骤)**：
    解压下载的 `.tar.bz2` 文件后，您会看到带有复杂版本号的文件名。**为了适配程序读取逻辑，请务必将它们重命名为以下固定名称**，并放入项目根目录下的 `models/` 文件夹中：

    | 原始文件 (示例) | **重命名为 (必须)** | 说明 |
    | :--- | :--- | :--- |
    | `...encoder...int8.onnx` | **`encoder.onnx`** | 编码器文件 |
    | `...decoder...onnx` | **`decoder.onnx`** | 解码器文件 |
    | `...joiner...int8.onnx` | **`joiner.onnx`** | 连接器文件 |
    | `tokens.txt` | **`tokens.txt`** | 词表文件 (无需改名) |
